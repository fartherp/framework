...
import static com.github.fartherp.framework.ip.Constant.IP_RECORD_LENGTH;
import static com.github.fartherp.framework.ip.Constant.REDIRECT_MODE_1;
import static com.github.fartherp.framework.ip.Constant.REDIRECT_MODE_2;
import static com.github.fartherp.framework.ip.Constant.UNKNOWN_AREA;
import static com.github.fartherp.framework.ip.Constant.UNKNOWN_COUNTRY;
...
public class IPUtils {
     ...
     */
    private static String readString(long offset) throws IOException {
        ipFile.seek(offset);
        int i = 0;
        ...
        if (i != 0) {
            return getString(buf, 0, i, Constant.GBK);
        }
    ...
    static {
        try {
            String name = IPUtils.class.getClassLoader().getResource(Constant.IP_FILE_NAME).getFile();
            ipFile = new RandomAccessFile(name, "r");
        } catch (FileNotFoundException e) {
            throw new RuntimeException("ip data file not found!", e);
        }
        // 如果打开文件成功，读取文件头信息
        try {
            ipBegin = readLong4(0);
